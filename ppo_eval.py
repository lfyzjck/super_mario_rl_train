...
if __name__ == "__main__":
    ...
    # autoset vecnormalize path if model path is provided
    if args.model_path is not None:
        current_chkp_step = args.model_path.split("_")[-2]
        vecnormalize_path = Path(args.model_path).parent / f"mario_model_vecnormalize_{current_chkp_step}_steps.pkl"
        if vecnormalize_path.exists():
            args.vecnormalize_path = vecnormalize_path
        else:
